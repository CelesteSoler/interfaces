class Tablero {
    ...
    /**
     * Dibuja los círculos apuntando a cada columna
     */
    dibujarCirculos() {
        this.canvaCtx.fillStyle = 'rgb(140, 200,0)';
        ...
        for (let i = 0; i < this.maxColumnas; i++) {
            ...
            const radius = this.anchoCelda/2.6;
            this.canvaCtx.beginPath();
            this.canvaCtx.arc(posX, posYInicio, radius, 0, 2 * Math.PI);
            this.canvaCtx.fill();
            this.canvaCtx.closePath();
            this.circuloTablero.push({ posXInicio: posX - 20, posXFinal: posX + 20, posYInicio: posYInicio, posYFinal: posYFinal });
    ...
    */
    ...
    obtenerColumna({ posX, posY }) {
        for (let i = 0; i < this.circuloTablero.length; i++) {
            const flecha = this.circuloTablero[i];
